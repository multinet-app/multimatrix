...
export default Vue.extend({
  ...
  methods: {
    ...
    async connectivityExample(this: any) {
      let start = this.connectivity.node1.value;
      let hop = this.connectivity.hop.value;
      let end = 'jfk';
      ...
      if (start === 'SLC') {
        if (hop === '30') {
          query = `FOR v, e, p IN 1 OUTBOUND 'nodes/SLC' GRAPH 'flights'
          FILTER v._key == 'JFK'
          FILTER p.edges.DEP_DELAY_MEAN <= '30'
          RETURN {nodes: p.vertices, links: p.edges}`;
        } else {
          query = `FOR v, e, p IN 1 OUTBOUND 'nodes/SLC' GRAPH 'flights'
          FILTER v._key == 'JFK'
          FILTER p.edges.AIRLINE == 'AA'
          RETURN {nodes: p.vertices, links: p.edges}`;
        }
      } else {
        if (hop === '30') {
          query = `FOR v, e, p IN 1 INBOUND 'nodes/JFK' GRAPH 'flights'
            FILTER e.DEP_DELAY_MEAN <= '30'
            RETURN {nodes: p.vertices, links: p.edges}`;
        } else {
          query = `FOR v, e, p IN 1 INBOUND 'nodes/JFK' GRAPH 'flights'
          FILTER e.AIRLINE == 'AA'
      ...
      this.network = await filterData(this.workspace, query);
    },
  },
});
</script>
    ...
    >
      ...
      <v-list class='pa-0'>
            ...
            >
              <v-card class='add-hops' tile>
                <v-card-title class='px-3 subtitle-2'
                  >Manage Nodes/Edges</v-card-title
                >
                ...
                <v-card-actions class='pa-3'>
                  <!-- DERYA -->
                  <v-btn
                    v-on:click='connectivityExample()'
                    color='primary'
                    block
                    depressed
                    large
        ...
        <v-list-item class='edge grey lighten-4'>
          <v-list-item-content>
            All edges
            <small>Use node/edge manager to specify filters.</small>
          </v-list-item-content>
        </v-list-item>
